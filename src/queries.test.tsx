...
import {addDefaultMutationFns, useSubmission, useSubmissionList} from "./queries";
...
test('useSubmission should return initial data from the list query', async () => {
  ...
  const { result: listResult } = renderHook(() => useSubmissionList({ limit: 10, offset: 0 }), { wrapper })
  ...
  const { result: submissionResult } = renderHook(() => useSubmission(TEST_ID_1), { wrapper })
...
test('useSubmission should mutate data', async () => {
  ...
  // First fetch data from the submission list query
  // TODO: fix hardcoded pagination options
  const { result: listResult, rerender: listRerender } = renderHook(() => useSubmissionList({ limit: 10, offset: 0 }), { wrapper })
  await waitFor(() => expect(listResult.current.isSuccess).toBe(true));
  ...
  expect(result.current.mutation.data?.metadata_submission.studyForm.studyName).toBe('UPDATED');
  expect(result.current.query.data?.metadata_submission.studyForm.studyName).toBe('UPDATED');
  ...
  listRerender();
  const submissionFromList = listResult.current.data?.results
    .find((submission) => submission.id === TEST_ID_1);
  expect(submissionFromList?.metadata_submission.studyForm.studyName).toBe('UPDATED');
});
...
test('useSubmission should replace paused mutations when offline', async () => {
  ...
  expect(result.current.query.data?.metadata_submission.studyForm.studyName).toBe('UPDATE 2');
})
...
test('useSubmission should rollback optimistic updates if a mutation fails', async () => {
  ...
  expect(result.current.query.data?.metadata_submission.studyForm.studyName).toBe('TEST 1');

  // The list should also have the rolled-back data
  const submissionFromList = listResult.current.data?.results
    .find((submission) => submission.id === TEST_ID_1);
  expect(submissionFromList?.metadata_submission.studyForm.studyName).toBe('TEST 1');
})
